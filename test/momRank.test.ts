...
const createTestData = async (): Promise<TestData> => {
  ...
  await createMom(conventionId, match1.body.id, player1Id, "Iniesta");
  await createMom(conventionId, match1.body.id, player1Id, "Iniesta");
  await createMom(conventionId, match1.body.id, player1Id, "Iniesta");
  await createMom(conventionId, match1.body.id, player1Id, "Pedri");
  await createMom(conventionId, match1.body.id, player2Id, "Kroos");
  await createMom(conventionId, match1.body.id, player2Id, "Kroos");
  await createMom(conventionId, match1.body.id, player2Id, "Fed");
  ...
  await createMom(conventionId, match2.body.id, player1Id, "Iniesta");
  await createMom(conventionId, match2.body.id, player1Id, "Gavi");
  await createMom(conventionId, match2.body.id, player3Id, "Koke");
...
describe('【正常系】GET /mom MOMランキングの取得', () => {
  ...
  it('特定の大会のMOMランキングが取得できる', async () => {
    ...
    try {
      ...
      expect(response.body[0].mom_count).toBe(4);
  ...
  it('特定のプレイヤー(チーム)のMOMランキングが取得できる', async () => {
    ...
    try {
      ...
      const response = await request(app)
        .get(`/api/mom?player_id=${testData.playerIds[1]}`)
        .send();

      expect(response.status).toBe(200);
      expect(response.body.length).toBe(2)
      expect(response.body[0].name).toBe("Kroos");
      expect(response.body[0].mom_count).toBe(2);
