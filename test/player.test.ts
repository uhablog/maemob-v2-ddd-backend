...
describe('【正常系】プレイヤーの作成', () => {
  ...
  it('【正常系】POST /player - 新しいプレイヤーが作成される', async () => {
    ...
    expect(response.body.draw).toBe(0);
    expect(response.body.lose).toBe(0);
    expect(response.body.goals).toBe(0);
    expect(response.body.conced).toBe(0);
  });
...
describe('【正常系】プレイヤーの取得', () => {
  ...
  it('全てのプレイヤーが勝ち点順で取得できる', async () => {
    ...
    expect(response.body.results[0].win).toBe(2);
    expect(response.body.results[0].draw).toBe(1);
    expect(response.body.results[0].lose).toBe(0);
    ...
    expect(response.body.results[1].win).toBe(1);
    expect(response.body.results[1].draw).toBe(1);
    expect(response.body.results[1].lose).toBe(1);
    ...
    expect(response.body.results[2].win).toBe(0);
    expect(response.body.results[2].draw).toBe(0);
    expect(response.body.results[2].lose).toBe(2);
    expect(response.body.results[2].goals).toBe(2);
    expect(response.body.results[2].concede).toBe(3);
  });
